...
import { getDtusOfPlant as getDtusOfPlantServe } from "@/services/plant";
...
const HighAnysis = () => {
    ...
    const initOption = async () => {
        ...
        if (dataType === "temp") {
            const fieldList = [
                intl.formatMessage({ id: "单体温度" }),
                intl.formatMessage({ id: "左侧熔断器温度" }),
                intl.formatMessage({ id: "右侧熔断器温度" }),
                intl.formatMessage({ id: "负极极柱温度" }),
                intl.formatMessage({ id: "正极极柱温度" }),
            ];
        ...
        if (res?.data?.data) {
            ...
            if (data) {
                ...
                if (currentPlantDevice?.length === 0) {
                    let packCellList = [];
                    const packCellListRes = await getAnalyticsInitDataServe({
                        dtuId: data?.[0]?.value,
                    });
                    ...
                    form.setFieldsValue({
                        ...
                        dataType: "vol",
                        packCell: [
                            packCellList?.[0]?.value,
                            packCellList?.[0]?.children?.[0]?.value,
                        ],
                    });
                    setTimeout(async () => {
                ...
                <Form
                    ...
                    onValuesChange={async value => {
                        if (value?.currentPlantDevice?.length === 2) {
                            let packCellList = [];
                            const packCellListRes = await getAnalyticsInitDataServe({
                                dtuId: value?.currentPlantDevice?.[1],
                            });
                ...
                >
                    <Flex align="center" wrap="wrap" gap="10">
                        ...
                        <Form.Item name={"dataType"} label={intl.formatMessage({ id: "数据项" })}>
                            <Select options={dataProList} style={{ width: "200px", height: 40 }} />
                        </Form.Item>
