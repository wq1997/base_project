...
const Index = ({ plants, activePlant, setActivePlant }) => {
    ...
    useEffect(() => {
        const _map =map|| new AMap.Map("map", {
            zoom: defaultZoom,
            center,
            // layers: [
        ...
        _map.on("complete", async () => {
            ...
            //  setMap(_map);
            addMarkers(_map, infoWindow, plants);
            positioning(plants?.[0]?.id);
        });
    }, [plants, theme]);
    ...
    const positioning = plantId => {
        console.log("map", map, plants);
        const moveTo = plants?.find(item => item.value == plantId)?.position;
