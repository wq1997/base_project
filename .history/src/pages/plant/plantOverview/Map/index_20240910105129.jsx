...
const Index = ({ plants, mapPlants, activePlant, setActivePlant }) => {
    ...
    useEffect(() => {
        ...
        _map.on("complete", async () => {
            ...
            addMarkers(infoWindow, mapPlants);
            positioning(mapPlants?.[0]?.id);
        });
    }, [mapPlants, theme]);

    const addMarkers = (infoWindow, mapPlants) => {
    ...
    const positioning = plantId => {
        console.log(mapObj, mapPlants);
        const moveTo = mapPlants?.find(item => item.value == plantId)?.position;
        if (!mapObj || !mapPlants) return;
        mapObj.panTo(moveTo);
        mmapObjap.setZoom(15);
        mapObj.setCenter(moveTo);
    };
