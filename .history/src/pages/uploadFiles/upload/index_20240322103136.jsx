...
const Company = ({ uploadOpen, onClose }) => {
    ...
    const [testFiles, setTestFiles] = useState([{ name: undefined }]);
    const [editData, setEditData] = useState();

    const addTestFiles = () => {
        setTestFiles([...testFiles, { name: undefined }]);
    };
            ...
            >
                ...
                >
                    {testFiles?.map((item, index) => {
                        return (
                            <Space style={{ marginBottom: "10px" }} key={index}>
                                <Input
                                    placeholder="最多输入30个字符"
                                    maxLength={30}
                                    style={{ width: "100%", marginRight: "20px" }}
                                    value={item.name}
                                    onChange={e => (testFiles[index].name = e.target?.value)}
                                />
                                <MyUpload
                                    url={uploadUrl}
