...
const ResCapTable = ({ data, onChange }) => {
    ...
    const checkValue = () => {
        const { heightPeakCut, lowPeakCut } = responsivenessDetail;
        const hValues = Object.values(heightPeakCut);
        const lValues = Object.values(lowPeakCut);
        if (hValues.includes("") || hValues.includes(undefined)) {
             message.info("请完整填写响应能力表格");
             
        }
        if (lValues.includes("") || lValues.includes(undefined)) {
    ...
    const change = (value, pKey, sKey) => {
        ...
        if(checkValue()) onChange(_responsivenessDetail);
    };
