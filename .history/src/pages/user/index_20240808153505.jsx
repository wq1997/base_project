...
import { Button, Space, Table, message, Modal, DatePicker, Radio, Input } from "antd";
...
const Account = () => {
    ...
    const [applicationId, setApplicationOpen] = useState(false);
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (_, { applicationStatus }) => {
                return (
                    <Space>
                        <a onClick={() => {setApplicationOpen(true)}}>审批</a>
                        <a onClick={() => {}} style={{ color: "#F5222D" }}>
                            删除
                        </a>
                    </Space>
                );
    ...
    return (
        <div>
            <Modal title="账号审批" open={applicationOpen} onOk={handleOk} onCancel={handleCancel}>
                <Radio.Group
                    onChange={e => {
