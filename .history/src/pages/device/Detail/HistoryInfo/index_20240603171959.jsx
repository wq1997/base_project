...
const Index = ({}) => {
    ...
    useEffect(() => {
        ...
        const keys = [...new Set(signalPoint?.map(item => JSON.parse(item)[1]))];
        console.log("keys", keys);
        const _signalPointOptions = signalPointOptions?.map(item => ({
            ...item,
            disabled: keys?.length == 2 && !keys.includes(item.keys) ? true : false,
        }));
        setSignalPointOptions(_signalPointOptions);
    }, [signalPoint]);
