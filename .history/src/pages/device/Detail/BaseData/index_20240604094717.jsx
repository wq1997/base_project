...
const Index = ({ deviceInfo }) => {
    ...
    useEffect(() => {
        ...
        Object.keys(deviceInfo?.realtimeList).forEach(key => {
            _dataSource[0].push(key);
            _dataSource[1].push(deviceInfo?.realtimeList?.[key]?.V);
            _dataSource[2].push(deviceInfo?.realtimeList?.[key]?.A);
        });
        setDataSource(_dataSource);
        setRealData(
            [...realData]?.map(item => ({
                ...item,
                value: deviceInfo?.inverterInfo?.[item.key],
            }))
        );
    }, [deviceInfo]);
