...
const Device = ({ isPowerOn, open, deviceInfo, onClose }) => {
    ...
    const onFinish = async values => {
        const res = await setDeviceServer({
            id: deviceInfo?.id,
            deviceConfig: "REACTIVE_POWER_ADJUST",
            ...values,
        });
    ...
    return (
        ...
        >
            <Form
                ...
                autoComplete="off"
            >
                <div style={{ marginBottom: "10px" }}>确认下发wu功调节指令？</div>
                ...
                <Form.Item
                    label="调节范围"
                    name="range"
                    initialValue={`${deviceInfo?.reactivePowerUpperLimit} ~ ${deviceInfo?.reactivePowerLowerLimit}`}
                >
                    <Input disabled={true} />
                </Form.Item>
