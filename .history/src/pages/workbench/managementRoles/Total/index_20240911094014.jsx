...
const Total = ({
    ...
    const allWorkorders = [
        { name: "在途异常工单", value: data?.inTransitWorkOrderSummery?.inTransitExceptionCount || 0, color: token.color16 },
        { name: "在途其他工单", value: data?.inTransitWorkOrderSummery?.inTransitOtherCount || 0, color: token.color17 },
    ];

    const myWorkorders = [
        { name: "发起工单数", value: data?.initiatedWorkOrderSummery?.initiatedCount || 0, color: token.color18 },
        { name: "已执行总数", value: data?.initiatedWorkOrderSummery?.initiatedByMeAndCompletedCount || 0, color: token.color19 },
    ];

    const todoList = [
        { name: "待执行工单总数", value: data?.todoWorkOrderSummery?.todoCount || 0, color: token.color20 },
        { name: "待执行异常工单", value: data?.todoWorkOrderSummery?.todoExceptionCount || 0, color: token.color21 },
    ];
    ...
    return (
        <div className="total"  className={styles.total}>
