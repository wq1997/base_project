...
const Confirm = () => {
    ...
    const getOptions = () => {
        if (!curTask) return;
        const {
            resourcePlan: { details: planDetails },
            dayaheadResourcePlan: { details: realDetails },
        } = curTask;
        const xData = [];
        const baseYData = [];
        const planYData = [];
        const realYData = [];
        resourcePlan?.de?.forEach((item, index) => {
            xData.push(item.timeRange);
            baseYData.push(item.basePower);
            planYData.push(item.targetPower);
            realYData.push(realDetails[index].targetPower);
        });
        setOptions({
            legend: {
