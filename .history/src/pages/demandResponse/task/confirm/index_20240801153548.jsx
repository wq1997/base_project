...
const Confirm = () => {
    ...
    const getTaskDashboard = async () => {
        const taskId = getUrlParams(search)?.taskId;
        const fn  =  taskId ? getTaskDetailServer :getWaitTaskListServer
        let res = await getWaitTaskListServer();
        if (res?.data?.status == "SUCCESS") {
            const list = res?.data?.data;
            const index = taskId ? list?.findIndex(item => item?.resourcePlan?.id == taskId) : 0;
            const curTask = taskId
                ? list?.find(item => item?.resourcePlan?.id == taskId)
                : list[index];
                console.log('----',curTask)
