import { Button, Space, message, theme as antdTheme } from "antd";
...
const Confirm = () => {
    ...
    const getTaskDashboard = async () => {
        ...
        if (res?.data?.status == "SUCCESS") {
            const { responseType, taskCapacity, projectedProfit, executeResult, projectedPowerData } = res?.data?.data;
            ...
            setBaseLinePower(projectedPowerData?.map(item => item?._1))
            setResponsePower(projectedPowerData?.map(item => item?._1))
            setTargetPower(projectedPowerData?.map(item => item?._1))
