...
const BaseLine = ({ baseLineArgs, onClose }) => {
    ...
    const Summary = () => {
        ...
        const baseYData = [];
        const runYData = [];
        baseLineArgs?.baseLineFullInfo?.forEach(item => {
            xData.push(item.timeRange);
            baseYData.push(item.baseLinePower);
            runYData.push(item.power);
        });
        const options = {
            legend: {
                data: ["基线功率", "执行功率"],
        ...
        return <ReactECharts option={options} style={{ width: "600", height: "600px" }} />;
    };
