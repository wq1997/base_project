...
const BaseLine = ({ baseLineArgs, onClose }) => {
    ...
    const getCompanyBaseLine = async () => {
        ...
        if (res?.data?.status == "SUCCESS") {
            const { energyStorageData, gatewayBaseLinePowers, baseLinePowersSource } =
                res?.data?.data;
            setEnergyStorageData(energyStorageData);
            setHistoryData(baseLinePowersSource);
            setSummaryData([{ name: "关口负载功率（KW）", data: gatewayBaseLinePowers },{ name: "储能计划出力功率（KW）", data: gatewayBaseLinePowers }]);
        }
    };
