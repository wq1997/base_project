...
const Account = () => {
    ...
    const initCode =paramsCode?decodeURIComponent(paramsCode)
    const { user } = useSelector(state => state.user);
    const [canDelete, setCanDelete] = useState(true);
    const releaseTimeRef = useRef();
    const executeTimeRef = useRef();
    const codeRef = useState(initCode ? decodeURIComponent(initCode) : null);
    const confirmStatusRef = useRef();
    const splitStatusRef = useRef();
    const responseTypeRef = useRef();
    const [code, setCode] = useState(initCode ? decodeURIComponent(initCode) : null);
    ...
    const getList = async () => {
        ...
        const code = codeRef.current || code;
