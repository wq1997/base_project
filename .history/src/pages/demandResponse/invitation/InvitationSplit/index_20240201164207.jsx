...
const Company = ({ invitationSplitId, onClose }) => {
    ...
    const getSplitInviteInitData = async () => {
        ...
        if (res?.data?.status == "SUCCESS") {
            ...
            setDeadline(invite?.tasks?.[0]?.confirmationDeadline);
    ...
    const isEqual = () => {
        let flag = false;
        for (let item of oldTaskList) {
            const newItem = taskList.find(newItem => newItem.companyCode == item.companyCode);
            console.log(item.companyCode,newItem.companyCode);
            if (!newItem) {
                return flag;
            } else {
                console.log( responsePower != newResponsePower)
                const { responsePower, confirmationDeadline } = item;
                const {
                    responsePower: newResponsePower,
