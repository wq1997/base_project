...
const Company = ({ invitationSplitId, showDetail, onClose }) => {
    ...
    const getDetail = async () => {
        ...
        if (res?.data?.status == "SUCCESS") {
            ...
            oldDeadline = invite?.confirmationDeadline;
            setDeadline(invite?.confirmationDeadline);
    ...
    useEffect(() => {
        if (invitationSplitId && !showDetail) {
            getSplitInviteInitData();
        }
    }, [invitationSplitId]);
    ...
    return (
        <>
            ...
            />
            <Modal
                title={showDetail ? "邀约详情" : isReSplit ? "重新拆分" : "邀约拆分"}
                width={1000}
