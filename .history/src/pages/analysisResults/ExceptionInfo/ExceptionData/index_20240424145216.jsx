...
const visualMap = () => {
    let points = [1, 13]; // 416 13
    let data = [];
    const max = 416;
    const groupLength = 13;
    const getRange = num => [num * groupLength - 12, num * groupLength];
    points.forEach((item, index) => {
        ...
        if (index != points?.length - 1) {
            const nextL = getRange(points[index + 1])[0];
            if (curR + 1 != nextL) {
                data.push({
                    color: "black",
                    range: [curR + 1, nextL - 1],
                });
            }
        } else {
            if (curR != max) {
