        ...
        if (res?.data?.code == 0) {
            ...
            if (
                ...
                for (let i in Array.from({ length: 416 })) {
                    const vNumber = `v${+i + 1}`;
                    const chargingItem = excSingleVoltages?.find(
                        item => item?.batteryStackCurrent > 0
                    );
                    const disChargingItem = excSingleVoltages?.find(item => item?.batteryStackCurrent < 0)
                    charging.push(chargingItem[vNumber]);
                    disCharging.push(
                        disChargingItem[vNumber]
                    );
                    labels.push(vNumber);
                    addColumns.push({
                        title: vNumber,
                ...
                setOptions({
                    ...
                    series: [
                        ...
                        },
                        {
                            ...
                            name: "放电",
                            markPoint: {
                                data: [{ value: 2, xAxis: 1, yAxis: 2.5 }],
                            },
                        },
                    ],
                });
