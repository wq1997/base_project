...
const ExceptionData = ({ infoId, onClose }) => {
    ...
    const getVisualMap = ({ groups, groupLength, maxNum }) => {
        let data = [];
        const getRange = num => [num * groupLength - groupLength, num * groupLength];

        groups.forEach((item, index) => {
            const [curL, curR] = getRange(item);
            console.log(getRange(item))
            if (index == 0 && curL != 0) {
                data.push({
                    color: "black",
                    gt: 0,
    ...
    const getInitData = async () => {
        ...
        if (res?.data?.code == 0) {
            ...
            if (
                ...
                const showPie = ['MODULE_VOLTAGE_STRATIFICATION_WITHIN_CLUSTER','MODULE_TEMPERATURE_STRATIFICATION_WITHIN_CLUSTER'].includes(sceneName)
                getOptions({
                    unit,
                    labels,
                    pieces: getVisualMap({
                        groups: exceptionScene?.physicalLocation?.split(","),
                        groupLength: 13,
                        maxNum: keys?.length,
                    }),
                    series,
