        ...
        if (res?.data?.code == 0) {
            ...
            if (
                ...
                excSingleVoltages?.forEach(item => {
                    ...
                    console.log(Array.from({ length: 416 })?.map(index => {
                        console.log(item[`v${+index + 1}`])
                        return item[`v${+index + 1}`]
                    }))
                    series.push({
                        ...itemStyle,
                        data: Array.from({ length: 416 })?.map(index => item[`v${+index + 1}`]), // isCharing ? charging : disCharging,
                        name: isCharing > 0 ? "充电" : "放电",
                        markPoint: {
                            data: matterPoints?.map(point => ({
                                value: `${point}`,
                                xAxis: point,
                                yAxis: item[point],
                            })),
