...
const ExceptionDetail = ({ exceptionDetailId, onClose }) => {
    ...
    const getInitData = async () => {
        const res = await getAnalysisResultsDetailServer(exceptionDetailId);
        if (res?.data?.code == 0) {
    ...
    useEffect(() => {
        setOpen(Boolean(exceptionDetailId));
        if (exceptionDetailId) {
            getInitData();
        }
    }, [exceptionDetailId]);
