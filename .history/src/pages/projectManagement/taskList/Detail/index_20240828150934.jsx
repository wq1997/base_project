...
const Detail = ({ detailId, processId, onClose }) => {
    ...
    const [isDetail, setType] = useState("detail");
    ...
    useEffect(() => {
        if (detailId || processId) {
            if (detailId) setType("detail");
            if (processId) setType("process");
            setOpen(true);
            getInfo();
        } else {
            setOpen(false);
    ...
    return (
        ...
        >
            ...
            {["MANUAL_FB_INSPECTION", "MANUAL_OTHER"].includes(info?.type) && (
                <ManualOrder type={type} info={info} onClose={onClose} />
            )}
            {/* 异常工单 */}
            {["SYS_EXCEPTION", "MANUAL_EXCEPTION"].includes(info?.type) && (
                <ExceptionOrder type={type} info={info} onClose={onClose} />
            )}
            {/* 实施工单 */}
            {["IMPLEMENT"].includes(info?.type) && (
                <ImplementOrder type={type} info={info} onClose={onClose} />
            )}
        </Drawer>
    );
};
