    ...
    return (
        ...
        >
            <Descriptions title="基础信息">
                ...
                <Descriptions.Item label="当前处理人">{info?.statusZh == "已完成" ? "" :info?.currentProcessorName;}</Descriptions.Item>
                <Descriptions.Item label="实际处理人">{info?.currentProcessorName}</Descriptions.Item>
                <Descriptions.Item label="发起人">{info?.initiatorName}</Descriptions.Item>
            </Descriptions>
            {/* 标准工单 */}
            {["CYCLE_INSPECTION", "MANUAL_INSPECTION"].includes(info?.type) && <StandardOrder />}
            {/* 非标准工单 */}
            {["MANUAL_FB_INSPECTION", "MANUAL_OTHER"].includes(info?.type) && <ManualOrder />}
            {/* 异常工单 */}
