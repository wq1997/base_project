...
import { 
    getElectricType as getElectricTypeServe,
    getElectricTypeByName as getElectricTypeByNameServe,
    updateBaseDataByType as updateBaseDataByTypeServe,
    addElectricType as addElectricTypeServe,
...
const ElectricityType = () => {
    ...
    const onSubmit = async () => {
        ...
        try {
            ...
            if(type==="Edit"){
                ...
                res = await addElectricTypeServe(values);
            }
            if(res?.data){
                getList();
                onCancel();
    ...
    const getList = async () => {
        ...
        if(!keyword){
            res = await getElectricTypeServe({
                current, 
                size: pageSize
            })
        }else{
            res = await getElectricTypeByNameServe({
                current, 
                size: pageSize,
                name: keyword
            })
            ...
            />
            {
                ...
                >
                    <Form
                        ...
                        labelCol={{span: 4}}
                    >
                        <Form.Item label="用电类型名称" name="name" rules={[{...FORM_FORBIDDEN_SPACE}, {...FORM_REQUIRED_RULE}]}> 
