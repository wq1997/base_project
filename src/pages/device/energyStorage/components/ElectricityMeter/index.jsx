...
const ElectricityMeter = () => {
    ...
    const gridPointId = getQueryString("gridPointId");

    const { token } = theme.useToken();
    const onChangeTab = key => {
        setActiveKey(key);
        history.push(`${pathname}?PageKey=${getQueryString("PageKey")}&pageType=${pageType}&activeKey=${key}&id=${id}&gridPointId=${gridPointId}`);
    ...
    return (
        <div style={{height: '100%'}}>
            ...
            <div className={styles.content} style={{backgroundColor: token.cardBgc,padding:'40px 30px',borderRadius: '0px 16px 0px 0px'}}>
                {activeKey==="RealtimeData"&&<RealtimeData id={id} gridPointId={gridPointId}/>}
